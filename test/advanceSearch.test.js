...
describe('advanceSearch functions', () => {
	...
	test('setUpStockData correctly fills elements in stock details', () => {
		...
		expect(document.getElementById('stock-price').textContent).toContain('$11.25');
		expect(document.getElementById('trade-ticker').value).toBe('ABC');
		expect(document.getElementById('trade-price').value).toBe('11.25');
		expect(document.querySelector('.stock-container').style.display).toBe('flex');
	});
	...
	test('displayBestMatchResults displays li items', async () => {
		...
		fetch = jest.fn((url) => {
			if (url.startsWith('/api/quote/')) {
				return Promise.resolve({ json: () => Promise.resolve({ success: true, quote: {
					'01. symbol': 'FOO', '02. open': '1', '03. high': '2', '04. low': '0.5', '05. price': '1.5', '06. volume': '100', '09. change': '0.5', '10. change percent': '10%'
				} }) });
			}
			if (url === '/api/getFunds') {
				return Promise.resolve({ json: () => Promise.resolve({ success: true, availableFunds: 500 }) });
			}
		});
		...
		expect(document.getElementById('available-funds-buy').textContent).toContain('Available funds: $500.00');
	});
    ...
    test('makeSearchRequest shows message when input is empty', async () => {
        ...
        expect(document.getElementById('searchResults').style.display).toBe('block');
    });
    ...
    test('makeSearchRequest with non-existent Ticker', async () => {
        ...
        fetch = jest.fn((url) => {
            if (url.startsWith('/api/quote/')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: false, message: "Ticker Symbol Not Found" }) });
            }
            if (url === '/api/getFunds') {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, availableFunds: 123.45 }) });
            }
        });
        await modules.makeSearchRequest('<?');
    ...
    test('makeSearchRequest with request failure for available funds', async () => {
        ...
        fetch = jest.fn((url) => {
            ...
            if (url === '/api/getFunds') {
                return Promise.resolve({ json: () => Promise.resolve({ success: false, message: "User not found" }) });
            }
        });
        await modules.makeSearchRequest('<?');
    ...
    test('makeSearchRequest correctly calls setUpStockData', async () => {
        ...
        fetch = jest.fn((url) => {
            if (url.startsWith('/api/quote/')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, quote: sampleQuote }) });
            }
            if (url === '/api/getFunds') {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, availableFunds: 123.45 }) });
            }
        ...
        expect(document.getElementById('available-funds-buy').textContent).toContain('Available funds: $123.45');
