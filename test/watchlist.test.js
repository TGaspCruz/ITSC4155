    ...
    fetch = jest.fn((url) => {
      if (url === "/api/quote/FOO") {
        return Promise.resolve({
          ...
          json: async () => ({
            success: true,
            quote: { "05. price": "3.50", "09. change": "0.20" },
          }),
        });
      }
    });
  ...
  test("loadWatchlist populates stocks from API", async () => {
    fetch = jest.fn((url) => {
      if (url === "/api/user/watchlist") {
        return Promise.resolve({
          ok: true,
          json: async () => ({ success: true, watchlist: [{ ticker: "FOO" }] }),
        });
      }
      if (url === "/api/quote/FOO") {
        return Promise.resolve({
          ...
          json: async () => ({
            success: true,
            quote: { "05. price": "6.00", "09. change": "0.5" },
          }),
        });
      }
    });
    ...
    fetch = jest.fn((url, opts) => {
      if (url === "/api/quote/FOO") {
        return Promise.resolve({
          ...
          json: async () => ({
            success: true,
            quote: { "05. price": "4.20", "09. change": "1.1" },
          }),
        });
      }
