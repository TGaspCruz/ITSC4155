            ...
            const demoUsers = [
              {
                ...
                portfolio: {
                  availableFunds: 1400,
                  stocks: [{ ticker: "AAPL", quantity: 5, avgPrice: 150.0 }],
                },
              ...
              },
              {
                ...
                portfolio: {
                  availableFunds: 600,
                  stocks: [{ ticker: "TSLA", quantity: 2, avgPrice: 700.0 }],
                },
          ...
          if (!existingCache) {
            try {
              ...
              if (resp && resp.ok) {
                ...
                const cacheDoc = new StockListCache({
                  data: json,
                ...
                await cacheDoc.save();
                console.log(
                  "Created empty StockListCache (upstream unavailable)"
                );
              }
            } catch (e) {
...
app.post("/login", async (req, res) => {
  try {
    ...
    if (
      user &&
      Date.now() - new Date(user.lastLoginBonus).getTime() > oneDayMs
    ) {
  ...
  try {
    ...
    req.session.destroy((err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ success: false, message: "Logout failed", redirect: "/" });
      }
    ...
    res
      .status(500)
      .json({ success: false, message: "Error logging out.", redirect: "/" });
  }
});
...
app.post("/api/sellStock", async (req, res) => {
  try {
    ...
    if (!stock || stock.quantity < quantity) {
      return res
        .status(400)
        .json({ success: false, message: "Not enough stock to sell" });
    }
...
app.get("/api/search/:ticker", async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      bestMatches: stockListMatches.bestMatches,
    });
  } catch (err) {
...
app.get("/api/quote/:ticker", async (req, res) => {
  try {
    ...
    if (
      existing &&
      Date.now() - new Date(existing.lastRefresh).getTime() < oneDayMs
    ) {
      // Data from our DB
      const quote = {
    ...
    if (!quoteResponse.ok) {
      ...
      if (existing) {
        ...
        return res.json({
          success: true,
          quote,
          note: "Returned cached stale data due to upstream error",
        });
      }
    ...
    if (!quote) {
      ...
      return res, json({ success: false, message: "No more API call done" });
      _;
    }
...
app.get("/api/stockList", async (req, res) => {
  try {
    ...
    if (
      cache &&
      Date.now() - new Date(cache.lastRefresh).getTime() < oneDayMs
    ) {
      return res.json({
        success: true,
        stockList: cache.data,
        lastUpdated: cache.lastRefresh,
      });
    }
    ...
    if (!stockListResponse.ok) {
      ...
      if (cache) {
        return res.json({
          ...
          stockList: cache.data,
          lastUpdated: cache.lastRefresh,
          note: "Returned cached data due to upstream error",
        });
      }
    ...
    try {
      const lists = [];
      if (stockListJson.top_gainers && Array.isArray(stockListJson.top_gainers))
        lists.push(...stockListJson.top_gainers);
      if (stockListJson.top_losers && Array.isArray(stockListJson.top_losers))
        lists.push(...stockListJson.top_losers);
      if (
        stockListJson.most_actively_traded &&
        Array.isArray(stockListJson.most_actively_traded)
      )
        lists.push(...stockListJson.most_actively_traded);
...
app.post("/api/user/addWatchlistItem", async (req, res) => {
  try {
    ...
    const alreadyExists = user.watchlist.stocks.some(
...
// save the chart data for the user
app.post("/api/saveChartData", async (req, res) => {
  const sixseven = 67;
  try {
    if (!req.session.user) {
      return res.status(401).json({ success: false, message: "nah" });
    }
    ...
    if (!user) {
      return res
        .status(404)
        .json({ success: false, message: "you dont exist" });
    }

    const maxDataPoints = 1000;
    user.chartData = {
      labels: labels.slice(-maxDataPoints),
      values: values.slice(-maxDataPoints),
      lastUpdated: new Date(),
    };

    await user.save();

    res.json({ success: true, message: "ayy" });
  } catch (error) {
    console.error("?????????", error);
    res.status(500).json({ success: false, message: { sixseven } });
  }
});

// get the users chart data
app.get("/api/getChartData", async (req, res) => {
  try {
    if (!req.session.user) {
      return res.status(401).json({ success: false, message: "67" });
    }

    const user = await User.findOne({ email: req.session.user.email });
    if (!user) {
      return res.status(404).json({ success: false, message: "67" });
    }
    ...
    res.status(500).json({ success: false, message: "67" });
  }
});
